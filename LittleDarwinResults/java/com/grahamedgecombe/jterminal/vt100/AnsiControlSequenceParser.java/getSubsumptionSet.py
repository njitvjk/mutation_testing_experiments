...
def loadBase():
	file=open('C:/Users/thiya/Downloads/mutationtesting-master/mutationtesting_01/LittleDarwinResults/java/com/grahamedgecombe/jterminal/vt100/AnsiControlSequenceParser.java/test_results/Base.txt');
...
def loadMutantKills(base_results,filename):
	file=open('C:/Users/thiya/Downloads/mutationtesting-master/mutationtesting_01/LittleDarwinResults/java/com/grahamedgecombe/jterminal/vt100/AnsiControlSequenceParser.java/test results/'+filename+'.txt');
	lines=file.readlines();
	data={};
	for line in lines:
		line=line.strip().split(",");
...
def loadMutant(filename):
	file=open('C:/Users/thiya/Downloads/mutationtesting-master/mutationtesting_01/LittleDarwinResults/java/com/grahamedgecombe/jterminal/vt100/AnsiControlSequenceParser.java/test_results/'+filename+'.txt');
	lines=file.readlines();
	data={};
	for line in lines:
		line=line.strip().split(",");
...
mutants=['m'+str(i) for i in range(1,95)]; #list of all mutants
mutants=[]
results={};
real_output={};
for m in mutants:
...
for m_a in mutants: #for each mutant m_a
	...
	for m_b in indistig_mutants: #for each main mutant in indisting classes
		if isSame(results[m_a],results[m_b]): #if m_a and m_b are indistiguishable, add m_a to class of m_b;
			indistig_mutants[m_b].append(m_a);
...
for m_a in main_mutants: #for each mutant m_a (one per indisting classes)
	...
	for m_b in main_mutants: #for each other mutant m_b from different indistng classes 
		if m_b==m_a:
			continue;
		subsumed_mutants[m_a][m_b]=MaSubsumesMb(results[m_a],results[m_b]);#if m_a subsumes m_b then subsumed_mutants[m_a][m_b]=True, it is Flase otherwise.
...
writer=open("C:/Users/thiya/Downloads/mutationtesting-master/mutationtesting_01/LittleDarwinResults/java/com/grahamedgecombe/jterminal/vt100/AnsiControlSequenceParser.java/analysis_subsumption.txt","w");
